...
{
	std::cout << _name << " has been constructed. Hyperion thank you for you purchase." << std::endl;
}
...
ClapTrap::~ClapTrap()
{
	std::cout << _name << " has been destructed. Hyperion also hates those annoying door openers." << std::endl;
}
...
void	ClapTrap::attack(std::string const & target)
{
	...
	std::cout << _name << " attacks " << target << ", dealing " << _AttackDamage << " damages." << std::endl;
}
...
void	ClapTrap::takeDamage(unsigned int ammount)
{
	if (_HitPoints == 0)
	{
		std::cout << "Man....  " << _name << "'s already broken to pieces..." << std::endl;
		return ;
	}
	std::cout << _name << " suffered " << ammount << " damages" << std::endl;
	if (_HitPoints - (int)ammount <= 0)
	{
		_HitPoints = 0;
		std::cout << "\"BWAAAAARGH...\"\n" << _name << " died horribly and painfully in a long agony. " << std::endl;
		return ;
	}
	_HitPoints -= ammount;
}
